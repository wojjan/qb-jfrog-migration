...
This repository contains Python (.py) and Windows batch (.bat) scripts used to compare
JFrog (jf) builds against Quick Build (qb) builds.

The Quick Build (qb) output is treated as the reference build, while the JFrog (jf)
build is the one being verified.
